...
**个人实名认证方式支持：人脸识别认证（支付宝人脸，腾讯云人脸），银行四要素认证，手机三要素认证。**

**企业实名认证方式支持：法定代表人认证，对公打款，法定代表人授权书，企业支付宝认证认证。**
...
+ **授权认证模式：**包含用户授权 + 实名认证认证。用户第一次使用e签宝需要做实名认证，证明真实身份。第二次及以后使用不需要再做实名，只需要验证码回填确认授权。
+ **实名认证模式：**用户只需要做一次，如果用户做过e签宝实名认证，再次调用接口使用实名模式，接口会提示："用户已实名"。
