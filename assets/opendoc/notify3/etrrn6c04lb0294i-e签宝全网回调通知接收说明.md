...
e签宝SaaS平台提供的全网数据变更推送功能。这意味着，除了在您的开发者应用平台内部触发的事件之外，用户在e签宝SaaS官网或其他任何对接e签宝SaaS产品的环境中进行的信息变更<font style="color:#DF2A3F;">（以下简称：e签宝SaaS账号下的信息变更）</font>，都将被实时推送至您的开发者平台，即为e签宝全网回调通知。

**2、哪些用户在e签宝SaaS账号下的信息变更，才能推送给您的开发者平台？**

    （1）开发者平台自身企业，即appId所属的企业自身，做了e签宝SaaS账号下的信息变更。

    （2）开发者通过[【获取机构认证&授权页面链接】](https://qianxiaoxia.yuque.com/opendoc/auth3/kcbdu7)接口获取的链接，让机构用户授权了《manage_org_member》或 《manage_org_resource》权限范围给开发者平台应用ID（appId）后，用户做了e签宝SaaS账号下的信息变更。

    （3）开发者通过[【e签宝官网的关联企业】](https://help.esign.cn/detail?id=kgkucwnnk6whzv82&nameSpace=cs3-dept%2Fexboae)添加其他分子公司或合作企业，并开通《组织管理》权限，其他分子公司或合作企业做了e签宝SaaS账号下的信息变更。

**3、目前具体支持哪些事件（e签宝SaaS账号下的信息变更）可通过e签宝全网推送？**

详见本文 [附1 Action事件列表](#juMxj)

<font style="color:#DF2A3F;">注：开发者应用下自主发起的事件（如签署、认证授权等相关通知）本身就会有应用的推送，不属于全网类推送事件，不在本文介绍。</font>

:::

## 1.e签宝全网回调通知概述
+ 首先，开发者配置回调通知Web Url地址。
+ 每当，相关事件发生时（如管理员转授成功），e签宝通知服务将会创建一个JSON对象，其中包含事件类型和与该事件相关的数据等信息。
+ 然后，e签宝通知服务通过 HTTP POST 请求将JSON对象发送到开发者配置的回调通知 Web Url中。
+ 开发者业务系统在收到回调通知后，可根据事件类型和相关数据做下一步的业务处理。

其流程描述如下：

![](https://cdn.nlark.com/yuque/__puml/0883c82944e84fc8b0ba869e21416bfd.svg)
...
开发者登录e签宝 [**开放平台**](https://open.esign.cn) 后点击【控制台】进入e签宝开发者控制台，在页面上方先选择【正式服务】<font style="color:#DF2A3F;">（沙箱环境则选择【沙箱服务】，其他流程一致）</font>，然后在页面下方左侧点击【应用管理】-【我的应用】后在右侧应用列表页面中点击【配置】进入“应用配置”页面，选择【Webhook】模块的“添加Webhook”按钮即可配置接收回调通知的URL，并需要在事件订阅中勾选对应事件。如下图：
...
![](https://cdn.nlark.com/yuque/0/2024/png/447795/1735277996955-0117fd2c-2432-4e52-a4d7-12afa53966a1.png)

![](https://cdn.nlark.com/yuque/0/2024/png/447795/1735288019941-3a65a590-ac70-421a-ae53-829a3ede34bf.png)

![](https://cdn.nlark.com/yuque/0/2024/png/447795/1735278554974-83a01dfc-8495-4904-929d-4861777d7240.png)
...

{
  "adminId": "7ffca**********0a8f6",
  "orgId": "3c4047**********134e7",
  "newAdminId": "0e04**********7c21e96",
  "timestamp": 1704954335352,
...
| DELEGATE_ADMIN | [管理员转授成功通知](https://qianxiaoxia.yuque.com/opendoc/notify3/klqut6l3u9dby693) |
| UPDATE_ORG_NAME | [企业信息变更通知](https://qianxiaoxia.yuque.com/opendoc/notify3/nxfqd2gudzhtmmh8) |
| UPDATE_LEGAL_REP | [法定代表人信息变更通知](https://qianxiaoxia.yuque.com/opendoc/notify3/cie6or1e856am9e7) |
| UPDATE_ORG_MEMBER | [企业成员变更通知](https://qianxiaoxia.yuque.com/opendoc/notify3/lmhlw4nlz46l3ohh) |
