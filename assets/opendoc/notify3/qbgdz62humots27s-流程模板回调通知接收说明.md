...
+ 每当，流程模板通知事件发生时（如模板填写完成），e签宝通知服务将会创建一个JSON对象，其中包含事件类型和与该事件相关的数据等信息。
+ 然后，e签宝通知服务通过 HTTP POST 请求将JSON对象发送到开发者配置的回调通知 Web Url中。
+ 开发者业务系统在收到回调通知后，可根据事件类型和相关数据做下一步的业务处理。

其流程描述如下：

![](https://cdn.nlark.com/yuque/__puml/cf01dd1eaf24c2b6878968505775af5b.svg)
...
[点击跳转 开放平台订阅回调通知配置方式](https://qianxiaoxia.yuque.com/opendoc/notify3/pmy852#d29GV)，勾选【v3版流程模板】相关配置项。

**方式二：接口设置：**

开发者可通过[【通过流程模板创建合同拟定和签署流程】](https://qianxiaoxia.yuque.com/opendoc/file-and-template3/megwsgkmpbg1tec1)接口中的`**<font style="color:#FA8C16;">notifyUrl</font>**`参数来配置签署类回调通知Url地址。<font style="color:#DF2A3F;">（接口设置不包含 </font>[流程模板创建完成通知](https://qianxiaoxia.yuque.com/opendoc/notify3/wtfka4flb5570t9b)、[签署流程发起结果通知](https://qianxiaoxia.yuque.com/opendoc/notify3/bnngmlz6ybsdgm48)<font style="color:#DF2A3F;">）</font>

### 2.3 接收并响应
#### 2.3.1 接收e签宝回调通知
当某个事件发生后，e签宝会主动 POST 请求开发者所设置的回调通知 Url 地址，并推送对应的事件数据信息。

例如：当用户完成模板填写操作后，e签宝将推送流程模板填写完成事件数据信息。

:::warning
**<font style="color:#E8323C;">【提示】</font>**

+ 若贵司有网络安全策略，请按照文末[附3 e签宝通知服务器信息](#KyEtR)配置防火墙，以便成功接收回调通知。
+ 若开发者长时间未收到e签宝的回调通知，可主动调用相关查询接口，以便获知流程当前进展状态。
...
（若中间重试通知成功，则中断不再继续重试）![](https://cdn.nlark.com/yuque/0/2024/png/447795/1706666689888-134f9a97-b213-44f7-9a76-447b9e674bd4.png)
...
| --- | --- |
| **<font style="color:#E8323C;">流程模板类通知事件</font>** | |
