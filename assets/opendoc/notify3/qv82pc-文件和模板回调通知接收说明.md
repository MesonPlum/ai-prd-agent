...
+ 每当，文件和模板通知事件发生时（如模板填写完成），e签宝通知服务将会创建一个JSON对象，其中包含事件类型和与该事件相关的数据等信息。
+ 然后，e签宝通知服务通过 HTTP POST 请求将JSON对象发送到开发者配置的回调通知 Web Url中。
+ 开发者业务系统在收到回调通知后，可根据事件类型和相关数据做下一步的业务处理。

其流程描述如下：

![](https://cdn.nlark.com/yuque/__puml/c510982a26b5862caecaed8f5f8d1f71.svg)
...
**方式一：开放平台Webhook事件订阅：****<font style="color:#DF2A3F;">（推荐）</font>**

[点击跳转 开放平台订阅回调通知配置方式](https://qianxiaoxia.yuque.com/opendoc/notify3/pmy852#d29GV)，勾选【v3版模板服务】相关配置项。

**方式二：接口设置：**

开发者可通过[【获取填写合同模板页面】](https://qianxiaoxia.yuque.com/opendoc/pdf-sign3/ub4ncy)接口中的`**<font style="color:#FA8C16;">notifyUrl</font>**`请求参数来配置文件和模板类回调通知Url地址。<font style="color:#DF2A3F;">（接口设置仅有模板填写通知，没有模板制作和编辑等其他通知）</font>

### 2.3 接收并响应
#### 2.3.1 接收e签宝回调通知
当某个事件发生后，e签宝会主动 POST 请求开发者所设置的回调通知 Url 地址，并推送对应的事件数据信息。

例如：当用户完成模板填写操作后，e签宝将推送文件模板填写完成事件数据信息。
...
```json
{
    ...
    "components": [
        {
            "componentId": "3d01532b***54a2c625",
            "componentValue": "[{\"row\":{\"column1\":\"zcy\",\"column2\":\"11\",\"column3\":\"22\",\"column4\":\"33\",\"column5\":\"44\"}},{\"row\":{\"column1\":\"01\",\"column2\":\"金额\",\"column3\":\"饿\",\"column4\":\"\",\"column5\":\"\"}},{\"row\":{\"column1\":\"\",\"column2\":\"\",\"column3\":\"\",\"column4\":\"\",\"column5\":\"好的\"}}]"
        },
        {
            "componentId": "d63fbd1d7***08c860889e",
            "componentValue": "测试账号"
        }
    ],
    "customBizNum": "01测试",
...
| --- | --- |
| **<font style="color:#E8323C;">文件和模板类通知事件</font>** | |
...
| GET_SEAL_POSITION | [获取签章位置信息通知](https://qianxiaoxia.yuque.com/opendoc/notify3/umvulch4szgrarh5) |
