:::warning
**<font style="color:#DF2A3F;">注：目前审批回调通知仅包含签署中企业印章的用印审批流程，</font>**[**点击了解 企业签署用印审批的触发与流程**](https://open.esign.cn/doc/opendoc/helper/qi453w)

:::

## 1.审批回调通知概述
+ 首先，开发者配置回调通知Web Url地址。
+ 每当，审批相关事件发生时（如发起审批流程），e签宝通知服务将会创建一个JSON对象，其中包含事件类型和与该事件相关的数据等信息。
+ 然后，e签宝通知服务通过 HTTP POST 请求将JSON对象发送到开发者配置的回调通知 Web Url中。
+ 开发者业务系统在收到回调通知后，可根据事件类型和相关数据做下一步的业务处理。

其流程描述如下：

![](https://cdn.nlark.com/yuque/__puml/32496ef07568c9a8504402170e227f25.svg)
...
[点击跳转 开放平台订阅回调通知配置方式](https://qianxiaoxia.yuque.com/opendoc/notify3/pmy852#d29GV)，勾选【v3版审批服务】相关配置项。

### 2.3 接收并响应
#### 2.3.1 接收e签宝回调通知
当某个事件发生后，e签宝会主动 POST 请求开发者所设置的回调通知 Url 地址，并推送对应的事件数据信息。

例如：当审批流程发起后，e签宝将推送该审批流的相关事件数据信息。

:::warning
**<font style="color:#E8323C;">【提示】</font>**

+ 若贵司有网络安全策略，请按照文末[附3 e签宝通知服务器信息](#KyEtR)配置防火墙，以便成功接收回调通知。
+ 若开发者长时间未收到e签宝的回调通知，可主动调用相关查询接口，以便获知审批流程当前进展状态。
