...
+ <font style="color:rgb(23, 26, 29);">当超过设置的印章授权有效期，导致印章授权失效。</font>

e签宝将根据开发者设置的[印章回调通知地址](https://qianxiaoxia.yuque.com/books/share/65b717a4-b957-4d56-b165-f6efd49e7144/ynu21s)，发送业务类型`**<font style="color:#FFA940;background-color:#E9E9E9;">action</font>**`为 **<font style="color:#E8323C;">"</font>****<font style="color:rgb(232, 50, 60);">SEAL_AUTH_INEFFICTIVE</font>****<font style="color:#E8323C;">" </font>**的回调通知，开发者可根据返回参数`**<font style="color:#FFA940;background-color:#E9E9E9;">authorizedPsnId</font>**`（被授权人账号ID）和`**<font style="color:#FFA940;background-color:#E9E9E9;">authorizedOrgId</font>**`（被授权机构账号ID）判断内部授权还是跨企业授权。

#### 回调参数
| **参数名称** | **必填** | **参数类型** | **参数说明** |
| --- | :---: | :---: | --- |
| action | 是 | string | 通知业务类型，固定值：**<font style="color:rgb(232, 50, 60);">SEAL_AUTH_INEFFICTIVE</font>** |
...
| authStatus | 是 | string | 授权状态（此回调通知只会有**0 -** 失效状态）<br/>**1** - 生效 ，** 0 -** 失效 ，** 2** - 已删除，**3** - 待生效 |
| authorizerOrgId | 是 | string | 授权机构账号ID<font style="color:#F5222D;">（委托单位）</font> |
| authorizedPsnId | 否 | string | 被授权人账号ID<font style="color:#F5222D;">（委托单位成员）</font><br/>+ **<font style="color:#F5222D;">企业内成员印章授权失效时，返回此字段</font>** |
| authorizedOrgId | 否 | string | 被授权机构账号ID<font style="color:#F5222D;">（受托单位）</font><br/>+ **<font style="color:#F5222D;">跨企业印章授权失效时，返回此字段</font>** |
