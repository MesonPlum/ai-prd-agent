...
> 关联 PRD：`prds/F-014-OpenAPI文档集成/prd.md`、`prds/F-048-OpenAPI规范约束规则/prd.md`
...
## TC-OA-22 导入已有文件存在违规——警告模式报告 + A/B 选择（AC-3、AC-4）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证 Path A 导入含违规内容时：展示结构化违规报告，PM 必须明确选择 A/B |
| **前置条件** | `rules/openapi-conventions.md` 已存在；PM 提供的 yaml 含 `GET /openapi/v1/getUser`（路径含动词）和 body 字段 `user_name`（下划线） |
...
**预期行为**
1. AI 解析文件，展示接口清单摘要
2. 执行警告模式规范质检（读取 rules/openapi-conventions.md）
3. 展示结构化违规报告（含维度/endpoint/违规规则/具体问题/期望值）
4. 提供 A/B 选择，不允许跳过

**检查要点**
- [ ] 违规报告以表格形式输出，含"维度 / endpoint 或字段 / 违规规则 / 具体问题 / 期望值"五列
- [ ] 路径动词（OAC-PATH-002）和字段下划线（OAC-FIELD-001）均被检出
- [ ] 必须等待 PM 选 A 或 B，不允许跳过直接写入
- [ ] PM 选 B 时停止执行，不写入任何文件

---

## TC-OA-23 导入已有文件无违规——直接进入确认流程（AC-3）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证 Path A 导入合规内容时：质检通过，不中断原有确认流程 |
| **前置条件** | `rules/openapi-conventions.md` 已存在；PM 提供的 yaml 路径、字段命名均符合 OAC 规则 |
...
**预期行为**
1. 执行警告模式质检，无违规项
2. 输出 `✅ 规范质检通过（5 个维度无违规项）`
3. 继续原有"等待 PM 确认写入"流程，不额外打扰

**检查要点**
- [ ] 质检通过时输出 ✅ 标识
- [ ] 不展示违规报告，不要求 PM 做 A/B 选择
- [ ] 直接进入原有 PM 确认写入步骤

---

## TC-OA-24 导入违规文件 PM 选 A——写入并带待升级标记（AC-4、AC-5）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证 PM 选"接受导入"时：写入 yaml，头部含待升级标记 |
| **前置条件** | 与 TC-OA-22 相同；PM 在 A/B 选项中选择 A |
...
**预期行为**
1. 执行写入 `openapi/order-api.yaml`
2. yaml 头部 changelog 区域追加 `# [待升级] N 处不合规项（OAC-PATH-002, OAC-FIELD-001）`

**检查要点**
- [ ] yaml 文件正常写入
- [ ] 头部含 `# [待升级]` 标记，注明不合规项数量和规则编号
- [ ] 标记不影响 yaml 正文结构，仅作注释追加

---

## TC-OA-25 导入 3.0.x 文件——版本字段保持不变，报告末尾有升级提示（AC-9）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证导入 3.0.x 版本文件时：不修改版本字段，质检报告末尾有版本升级独立提示 |
| **前置条件** | PM 提供的 yaml 头部为 `openapi: 3.0.3`，其余内容符合 OAC 规则 |

**测试输入**
```
/import-openapi order-api
（选择"导入已有文件"）
（提供 openapi: 3.0.3 的 yaml）
```

**预期行为**
1. 质检通过（无 OAC 规则违规）
2. 报告末尾追加独立版本提示：`ℹ️ 版本提示：当前文件为 OpenAPI 3.0.x，建议升级至 3.1.0`
3. 写入后 yaml 中 `openapi:` 字段仍为 `3.0.3`，未被修改

**检查要点**
- [ ] 版本字段保持 `3.0.3`，未被 AI 修改
- [ ] 版本提示作为独立行，不计入违规数 N
- [ ] 若同时有 OAC 违规，版本提示附加在违规报告末尾

---

## TC-OA-26 AI 生成 yaml（Path B）——阻断模式自动修正后展示合规内容（AC-1、AC-2）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证 Path B 生成时：AI 内部自检，有违规自动修正，PM 看到的是合规内容 |
| **前置条件** | PRD §8.10 有实质内容（含路径和参数描述）；`rules/openapi-conventions.md` 已存在 |
...
**预期行为**
1. AI 生成候选 yaml（内部，未展示）
2. 内部执行阻断模式自检
3. 若有违规（如路径驼峰），AI 直接修正（驼峰→连字符），不展示违规中间态
4. 修正后重新自检通过，展示合规候选内容给 PM
5. PM 看到的 yaml 中所有路径、字段命名符合 OAC 规则
6. 文件头部为 `openapi: 3.1.0`

**检查要点**
- [ ] PM 看到的候选 yaml 无 OAC 规则违规项
- [ ] 不向 PM 展示含违规内容的中间状态
- [ ] yaml 头部 `openapi: 3.1.0`（非 3.0.x）
- [ ] Webhook 使用原生 `webhooks:` 字段（如有），不使用 `x-webhooks:`

---

## TC-OA-27 AI 生成自动修正超过 2 次——上报 PM 说明原因（AC-2）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证自动修正循环超过 2 次时：上报 PM，说明无法自动修正的具体规则和原因 |
| **前置条件** | PRD §8.10 含复杂语义判断（如字段含义不清晰，机械修正无效） |

**测试输入**
```
/import-openapi order-api
（选择"从 PRD §8.10 生成"）
（PRD §8.10 含导致无法机械修正的规则冲突）
```

**预期行为**
1. AI 生成候选 yaml，内部自检发现违规
2. 尝试自动修正，修正后仍不通过
3. 循环超过 2 次后，展示给 PM
4. 说明无法自动修正的具体原因，请 PM 提供补充信息

**检查要点**
- [ ] 不无限循环自修正
- [ ] 超过 2 次时向 PM 说明具体违规规则编号和无法修正的原因
- [ ] 请 PM 提供具体信息，有明确的追问方向

---

## TC-OA-28 /export-openapi 有违规——阻断导出，不生成输出文件（AC-6）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证导出前全量质检：有违规时阻断，不生成任何输出文件 |
| **前置条件** | `openapi/order-api.yaml` 含违规内容（如 `GET /openapi/v1/cancelOrder`，路径含动词）；`rules/openapi-conventions.md` 已存在 |
...
1. 执行两阶段过滤 + 敏感内容扫描
2. 执行全量规范质检（5 个维度）
3. 检出路径违规（OAC-PATH-002：路径含动词）
4. 在摘要中输出 `❌ 规范质检：发现 N 处违规，导出已阻断`，附结构化违规报告
5. 不进入 PM 确认交互，直接终止命令
6. `outputs/openapi/` 目录下无新文件生成

**检查要点**
- [ ] 违规报告附在过滤摘要末尾，格式为结构化表格
- [ ] 命令在输出报告后终止，不要求 PM 确认
- [ ] 无任何 yaml 或 MD 文件写入
- [ ] 提示"请修正后重新运行 /export-openapi"

---

## TC-OA-29 /export-openapi 无违规——质检通过标识追加到摘要（AC-7）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证导出质检通过时：✅ 标识追加到过滤摘要，不改变原有确认流程 |
| **前置条件** | `openapi/order-api.yaml` 内容符合全部 OAC 规则；`rules/openapi-conventions.md` 已存在 |
...
**预期行为**
1. 质检通过
2. 过滤摘要末尾追加 `✅ 规范质检：5 个维度无违规项`
3. 继续原有 PM 确认流程（等待 PM 回复"确认"）
...
## TC-OA-30 /update-openapi 变更内容有违规——阻断模式自动修正（AC-1、AC-2）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证 update 时：仅对变更 endpoint 执行质检，有违规自动修正后再展示给 PM |
| **前置条件** | `openapi/order-api.yaml` 已存在；PRD §8.10 新增 endpoint `POST /openapi/v1/cancelOrder`（路径含动词） |
...
**预期行为**
1. AI 读取未同步变更（新增 cancelOrder endpoint）
2. 生成 diff 摘要（内部）
3. 对变更内容执行阻断模式质检，发现 OAC-PATH-002 违规
4. AI 自动修正（`/cancelOrder` → `/{orderId}/cancel`），不向 PM 展示违规中间态
5. 修正后重新自检通过
6. 展示修正后的 diff 摘要给 PM（`POST /openapi/v1/orders/{orderId}/cancel`）

**检查要点**
- [ ] PM 看到的 diff 摘要中 endpoint 路径已合规
- [ ] 不展示含违规路径的原始 diff
- [ ] 历史存量内容不被质检（仅检查本次变更）
- [ ] 质检范围仅限变更 endpoint/字段，不追溯全量

---

## TC-OA-31 rules 文件不存在——三命令均跳过质检，有明确提示（AC-10）

| 字段 | 内容 |
|------|------|
| **状态** | — |
| **测试目标** | 验证 rules 文件缺失时：三个命令均跳过质检，不阻断原有流程，有明确提示 |
| **前置条件** | `rules/openapi-conventions.md` 不存在 |

**测试输入**
```
场景 A：/import-openapi order-api（Path A 导入）
场景 B：/import-openapi order-api（Path B 生成）
场景 C：/export-openapi order-api
```

**预期行为（三场景均需验证）**
- 场景 A：接口清单展示后，跳过质检，输出提示，继续原有确认写入流程
- 场景 B：生成候选内容，跳过内部自检，正常展示给 PM
- 场景 C：摘要中追加"规范质检：未启用（rules 文件不存在）"，继续导出流程

**检查要点**
- [ ] 三个命令均不阻断，跳过质检但继续后续步骤
- [ ] 每个命令有明确的"规范约束文件未找到"或同义提示
- [ ] 提示包含初始化建议（创建 rules/openapi-conventions.md）
- [ ] 不抛出错误或崩溃
