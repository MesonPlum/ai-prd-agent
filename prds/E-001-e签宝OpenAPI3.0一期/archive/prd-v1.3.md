...
has-prototype: false
req-ids: []
notes: "原始文档 V0.1（2022-02-22）已归档至 archive/original-v0.1.md；v1.1 已对比 assets/opendoc/ 确认所有接口上线状态，原文'待定'/'二期'标注已全部更新。"
---
...
| F-002 | file-and-template3 | 文件&流程模板管理 | Must | 待创建 | - |
| F-003 | pdf-sign3 | 签署流程 | Must | 待创建 | - |
| F-004 | account3 / console | 用户资源管理（认证信息查询、合同管理、企业控制台） | Must | 待创建 | - |
...
| 阶段 | 包含功能 | 目标 | 计划时间 |
|------|----------|------|----------|
| 一期 MVP | F-001、F-002（文件模板）、F-003（基础签署流程）、F-004 | 完整签署闭环可用，合规体系建立 | 2022-03（原计划） |
...
**认证信息查询**（同 F-001 auth3 中的认证查询接口，见上）：
...
> **V3 演变说明**：原文"查询个人/机构合同列表"在 V3 未设计为独立的"合同列表"接口，改为通过签署流程列表查询实现；account_3 模块在 V3 中定位为账号凭证管理，不包含合同查询能力。

| 接口 | V3 对应 opendoc 文件 | 状态 |
|------|---------------------|------|
| 查询签署流程列表（替代"查询个人合同列表"） | pdf-sign3/kq4b2e | ✅ 已上线 |
| 查询集成方企业流程列表（替代"查询机构合同列表"） | pdf-sign3/uhma1i | ✅ 已上线 |
| 下载已签署文件及附属材料 | pdf-sign3/kczf8g | ✅ 已上线 |
...
| 接口 | V3 对应 opendoc 文件 | 状态 |
|------|---------------------|------|
| 修改/新增账号登录凭证 | account_3/vriragi8ohekdme0 | ✅ 已上线 |
| 解绑账号登录凭证 | account_3/smelh6m20a5fizu2 | ✅ 已上线 |
...
| 认证授权（F-001）| 用户资源管理（F-004）| 查询用户认证/合同信息需先授权 |
