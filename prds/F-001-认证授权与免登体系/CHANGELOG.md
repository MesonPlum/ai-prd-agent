...
**变更内容**：
1. BR-04 规则补充：实名认证模式下（`authorizedScopes` 为空或未传 `authorizeConfig`），若用户**未实名**则正常返回 `authUrl` 让用户进行实名，若用户**已实名**则报错
2. §8.3 异常处理表更新：触发条件描述统一为"`authorizedScopes` 为空或未传 `authorizeConfig`"，并增加"企业名称与法人身份信息同时与 e签宝 已有不一致时，实名后不会生效"的说明
3. §8.4 业务流转图调整：先判断经办人是否实名，经办人实名后再判断企业是否实名，全部实名后进入授权模式判断，经办人是管理员或法人则直接授权完成，否则需要管理员审批

**影响范围**：§5.3 BR-04 规则定义、§8.3 异常处理要求、§8.4 业务流转图
